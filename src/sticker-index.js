...
async function buildStickerIndexForLibrary(libraryId, defaultLibName, document, progressReporter) {
  ...
  // first, adding all the symbols from the library
  if(SHOW_DEFAULT_STICKERS) {
    let allSymbolMasters = util.getAllLayersMatchingPredicate( document, NSPredicate.predicateWithFormat('className == %@', 'MSSymbolMaster'));
    allSymbolMasters.reverse();

    let symbolStickersMetaData = [];
    let metaData = {};
    metaData.name = "@" + _defaultSection;
    symbolStickersMetaData.push(metaData);

    for (let symbolLayer of allSymbolMasters) {
      // var symbolInstance = symbolLayer.newSymbolInstance();
      var symbolInstance = symbolLayer;
  ...
  // log("combinedText");
  // log(combinedText);
  let tempParsedMetadata = parseStickerMetadata(combinedText);
  ...
  libraryIndex.sections.push(libraryIndex.sections.shift());
  ...
  for (let layer of possibleStickers) {
    ...
    if (parsedName.isSticker || (layer instanceof MSSymbolMaster)) {
      ...
      for (let sectionId of parsedName.sectionIds) {
        ...
        let found = section.items.some(el => el.id === id);
        if(!found) {
  ...
  let nonEmptyItems = items => items.filter(item => {
    if (item.type == 'layer' || item.type == 'symbol') {
      return true;
...
function getStickersMetadata(stickersMetaData) {
  ...
  stickersMetaData.forEach(stickerMetaData => {
    ...
    var sectionTitle = stickerMetaData.name.split('.')[1];
    if (sectionTitle) {
    ...
    if(sectionTitle == _defaultSection) {
      yamlStickersMetadata += `${description}\n`;
    } else {
      yamlStickersMetadata += `${layout}\n`;
      yamlStickersMetadata += `${backgroundEach}\n`;
    }

    yamlStickersMetadata += `\n`;
  });
...
function getCategoryName(name) {
  ...
  var listOfCategoryItems = ['small', 'medium', 'large'];
  var newName = '';
